module.exports = (app) => {
  app.post("/fb-login", (req, res) => {
    ...
    .then(user => {
      var token = app.jwt.sign({ _id: user._id, email: user.email, password: user.password }, app.config.secret, {
        expiresIn: 86400 // expires in 24 hours
      })
      res.json({message: "success", token, data: user, wordsOfTheDay: app.wordsOfTheDay})
    })
    .catch((error) => {
      console.log(error)
